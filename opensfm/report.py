...
class Report:
    ...
    def make_dataset_summary(self):
        ...
        rows = [
            ...
            ["Uçuş Başlangıç", self.stats["processing_statistics"]["start_date"]],
            ["Uçuş Bitiş", self.stats["processing_statistics"]["end_date"]],
        ]
    ...
    def make_processing_summary(self):
        self._make_section("Analiz Özeti")
        ...
        if self._has_meaningful_gcp():
            geo_string.append("YKN")

        ratio_shots = rec_shots / init_shots * 100 if init_shots > 0 else -1
        rows = [
            [
                "Kullanılan Fotoğraflar",
                f"{init_shots}'da  {rec_shots} adet ({ratio_shots:.1f}%)",
            ],
            [
                "Oluşturulan/Kullanılan Noktalar (Seyrek)",
                f"{init_points}'da {rec_points} nokta ({rec_points/init_points*100:.1f}%)",
            ],
            # [
            ...
            # ],
            [
                "Tespit Edilen Öznitelikler",
                f"{self.stats['features_statistics']['detected_features']['median']:,} öznitelik",
            ],
            [
    ...
    def make_processing_time_details(self):
        self._make_section("Analiz Süresi Detayı")
    ...
    def make_gps_details(self):
        ...
        if table_count > 0:
            ...
            if a_ce90 > 0 and a_le90 > 0:
                rows += [[
                    "Yatayda Doğruluk CE90 (metre)",
                    f"{a_ce90:.3f}",
                    f"{r_ce90:.3f}" if r_ce90 > 0 else "-",
                ],[
                    "Düşeyde Doğruluk LE90 (metre)",
                    f"{a_le90:.3f}",
    ...
    def make_features_details(self):
        self._make_section("Öznitelik Detayları")
    ...
    def make_reconstruction_details(self):
        self._make_section("İnşa Detayları")

        rows = [
            [
                "Ortalama Yansıtma Hatası (normalize / pixel / açısal)",
                (
    ...
    def make_camera_models_details(self):
        self._make_section("Kamera Modeli Detayları")
    ...
    def make_tracks_details(self):
        self._make_section("Hat Detayları")
    ...
    def make_survey_data(self):
        self._make_section("Örtüşme Detayı (Fotoğraf Sayısı)")
    ...
    def make_preview(self):
        ...
        if os.path.isfile(ortho) or os.path.isfile(dsm):
            ...
            if os.path.isfile(dsm) and self.stats.get('dsm_statistics'):
                ...
                self._add_image_label("Dijital Yüzey Modeli (DSM)")
            ...
            if os.path.isfile(dtm) and self.stats.get('dtm_statistics'):
                ...
                self._add_image_label("Dijital Arazi Modeli (DTM)")
